...
// NOT IN USE
TArray<ATile*>& AChessPiece::GetAllPossibleTiles(ABoard*& GameBoard, bool IsAI)
{
...
{
	UE_LOG(LogTemp, Warning, TEXT("AI_TestMove begin"))
	if (!NewTile || !CurrentTile) { return; }
	...
	// If capturing add new nullptr piece and bool to true on tile to will move to.

	// On moving add this and bool to false on tile to move to.
	...
	if (NewTile->GetIsPossibleMoveLocation())
	{
		...
		{   
			...
			UE_LOG(LogTemp, Warning, TEXT("Adding nullptr and true"), *NewTile->GetTileName().ToString())
			// "Remove" current piece from NewTile and true for capture
			AChessPiece* Piece = nullptr;
			NewTile->AddNewSavedInfo(Piece, true);
		}
		else
		{
			//UE_LOG(LogTemp, Warning, TEXT("AI_TestMove: Moving %s %s from %s to %s:"), *Color, *GetName(), *CurrentTile->GetTileName().ToString(), *NewTile->GetTileName().ToString())
			 UE_LOG(LogTemp, Warning, TEXT("Adding nullptr and false"), *NewTile->GetTileName().ToString())
			
			// "Remove" current piece from NewTile and true for capture
			 AChessPiece* Piece = nullptr;
			 NewTile->AddNewSavedInfo(Piece, false);
		}

		UE_LOG(LogTemp, Warning, TEXT("AI_TestMove: Removing this(%s) from CurrentTile(%s)! Adding nullptr and false"), *GetName(), *CurrentTile->GetTileName().ToString())
		// "Remove" current piece from current tile and false for no capture
		AChessPiece* Piece = nullptr;
		CurrentTile->AddNewSavedInfo(Piece, false);
	...
	UE_LOG(LogTemp, Warning, TEXT("AI_TestMove end"))
...
void AChessPiece::AI_UndoTestMove(FMove*& Move, ABoard *& GameBoard, bool IsLastRegularUndo)
{
	...
	if (LastTiles.Num())
	{
		CurrentTile->RemoveLastSavedInfo();
 ...
 //
	...
	//	else
	//	{
	//		UE_LOG(LogTemp, Warning, TEXT("AI_UndoTestMove: It does not equal."))
	//	}
	//}
	//else
	//{
